# AuditEngine — Kimi Code Standing Orders
# This file is merged into the system prompt for every session in this project.
# Do not delete or move. Keep it in the project root.
...
1. Read BUILD_STATE.md first — understand what is done, what is in progress, what is pending.
...
**Workers runtime:** Never import node built-ins (`fs`, `path`, `child_process`, etc.).
The Workers runtime has no filesystem and no node_modules. Use Web APIs and R2 for storage.
...
## WORKFLOW REMINDERS
- The build plan is in AUDITENGINE_BUILD_BIBLE.md.
- The operator command cheatsheet is in SESSION_COMMANDS.md.
- Every planned session has a corresponding skill under `.kimi-code/skills/` (e.g., `/skill:auditengine-s00`).
- The human must run `git add -A && git commit -m "S0X: ..."` after each session.
