...
| S57 | 2026-08-18 | Gap remediation: all 19 agents spawned, remediation planner, AI fix agent, continuous audit cron trigger, error surfacing, secret hygiene; deployed to workers.dev | src/workers/coordinator.ts, src/agents/base-agent.ts, src/lib/default-constitution.ts, scripts/upload-constitutions.sh, src/workers/planner.ts, src/workers/fix-agent.ts, src/workflows/priority-resolver-workflow.ts, src/index.ts, wrangler.toml, src/db/schema.sql, src/db/migrate.ts, src/db/migrations/v14_task_plans.sql, src/workers/ingestion.ts, src/dashboard/task-board-html.ts, src/types/index.ts, src/lib/model-router.ts, test/planner.test.ts, test/fix-agent.test.ts, test/workflows.test.ts, BUILD_STATE.md | 034db37 |

## S57 — GAP REMEDIATION: AGENT ROSTER, PLANNER, FIX AGENT, CRON, ERROR SURFACING

| File | Status | Notes |
|------|--------|-------|
| src/workers/coordinator.ts | ✅ | All 19 agent types now spawned (phase-1: architecture/database/configuration/dependency; phase-2: security/api/frontend/backend/devops/error_handling; phase-3: documentation/performance/testing/logging/code_quality/a11y/i18n/refactoring); spawning detached via ctx.waitUntil so alarms no longer block on full agent lifecycles; agents in 'error' state count as finished for phase progression |
| src/agents/base-agent.ts | ✅ | Agent DO loop wrapped in try/catch — crashes now set registry status='error', write agent_errors + audit_logs, broadcast agent_error, and return 500 instead of failing silently; claims insert now includes tenant_id; missing R2 constitution falls back to embedded default constitution |
| src/lib/default-constitution.ts | ✅ | New — generic fallback constitution preserving evidence/schema rules |
| scripts/upload-constitutions.sh | ✅ | New — `npm run upload-constitutions` pushes src/constitutions/*.md + SYSTEM_SPEC.md to R2 |
| src/workers/planner.ts | ✅ | New — AI remediation plans per task (plan_text/plan_status), POST .../tasks/:id/plan, auto-generated for top 20 tasks after priority resolution |
| src/workers/fix-agent.ts | ✅ | New — POST .../tasks/:id/fix generates corrected files with the LLM, commits to branch auditengine/fix-<id>, opens PR/MR via git-write, moves task to in_review and triggers verification |
| src/workflows/priority-resolver-workflow.ts | ✅ | Added generate-remediation-plans step |
| src/index.ts | ✅ | Wired /plan and /fix routes; added scheduled() handler starting ContinuousAuditWorkflow hourly per completed repo audit |
| wrangler.toml | ✅ | Added [triggers] crons; removed invalid schedules key from [[workflows]]; secrets removed from [vars] (must use wrangler secret put) |
| src/db/schema.sql, src/db/migrate.ts, src/db/migrations/v14_task_plans.sql | ✅ | tasks.plan_text + plan_status columns (idempotent ALTER for existing DBs) |
| src/workers/ingestion.ts | ✅ | tagDomain extension fallbacks (py/go/java/etc → backend, yaml/toml/ini → config, broader test detection) so non-JS repos reach specialists |
| src/dashboard/task-board-html.ts | ✅ | "AI Plan" and "Apply Fix → PR" buttons per task card + plan modal |
| src/types/index.ts, src/lib/model-router.ts | ✅ | New TaskTypes remediation_plan/code_fix with routes; Task.plan_text/plan_status; agent_error/plan_ready/fix_applied events |
